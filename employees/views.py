...
@login_required
def delete_current_employee(request):
    try:
        user = request.user
        employee = get_object_or_404(Employee, user=user)

        # First delete the employee profile
        employee.delete()
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            leave_data = [
                {
                    ...
                    "status": leave.status,
                    "appliedDate": leave.created_at.strftime("%Y-%m-%d"),
                    "reason": leave.reason
                }
                for leave in leaves
            ]
            ...
            if not leave_id or not new_status:
                return JsonResponse({"status": "error", "message": "Missing leave_id or status"}, status=400)

            leave = get_object_or_404(LeaveApplication, leave_id=leave_id, employee=employee)

            if new_status not in dict(LeaveApplication.STATUS_CHOICES):
                return JsonResponse({"status": "error", "message": "Invalid status"}, status=400)

            leave.status = new_status
            leave.save()

            return JsonResponse({"status": "success", "message": f"Leave {leave_id} marked as {new_status}"})
...
def export_attendance_to_csv(request):
    try:
        records = AttendanceRecord.objects.select_related("user__user", "company").all()

        data = []
        for record in records:
            data.append({
                "Employee": record.user.user.username,
